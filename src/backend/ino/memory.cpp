...
memory::memory (port<dynInstruction*>& execution_to_memory_port, 
	...
	: stage (memory_width, stage_name, clk),
      ...
      _mshr (20, 4, 4, clk, g_cfg->_root["cpu"]["backend"]["table"]["mshr"], "MSHR"),
      _st_buff (20, 4, 4, clk, g_cfg->_root["cpu"]["backend"]["table"]["st_buff"], "stBuff"),
      _L1 (1, 64, 32768),
      _L2 (1, 64, 2097152),
      _L3 (1, 64, 8388608),
...
void memory::completeIns () {
    for (WIDTH i = 0; i < _stage_width; i++) {
        ...
        dynInstruction* mem_ins = _mem_buff.getFront ();
        if (!g_RF_MGR->hasFreeWire (WRITE, mem_ins->getNumWrAR ())) break; //TODO too conservatice - only for load ins

        /* COMPLETE INS */
        mem_ins = _mem_buff.popFront ();
...
PIPE_ACTIVITY memory::memoryImpl () {
    ...
    for (WIDTH i = 0; i < _stage_width; i++) {
        ...
        CYCLE axes_lat;
        if (mem_ins->getMemType () == STORE) {
            if (g_cfg->isEnSquash ()) Assert (mem_ins->isOnWrongPath () == false);
