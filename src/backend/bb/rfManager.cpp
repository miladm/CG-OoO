...
bb_rfManager::bb_rfManager (WIDTH num_bbWin, sysClock* clk, string rfm_name = "bb_rfManager")
    : unit (rfm_name, clk),
...
bb_rfManager::~bb_rfManager () { 
    WIDTH num_bbWin = _LRF_MGRS.size ();
...
void bb_rfManager::updateReg (PR reg) { //TODO fix this interface
//    WIDTH bbWin_id = ins->getBBWinID ();
    return _LRF_MGRS[bbWin_id]->updateReg (reg);
}
...
void bb_rfManager::squashRegs (INS_ID squashSeqNum) {
    _GRF_MGR->squashRenameReg ();
    dbg.print (DBG_REG_FILES, "%s: %s (cyc: %d)\n", _c_name.c_str (), 
            "Squashed GRF", _clk->now ());

    for (int i = 0; i < _LRF_MGRS.size (); i++) {
        _LRF_MGRS[i]->resetRF ();
