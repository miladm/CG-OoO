...
uint64_t cache::getIndex(uint64_t addr)
{
    ...
    uint64_t tempAddr = addr >> (3+3+3); //3b for each of byte-offset, word-offset, block-offset
...
{
    uint64_t tempAddr = addr >> (3+3+3); //3b for each of byte-offset, word-offset, block-offset
    tempAddr = tempAddr << (3+3+3);
    uint64_t tempIndex = getIndex(addr);
    tempIndex = tempIndex << (3+3+3); //byte+word+block offset
    return (tempAddr - tempIndex);
}

int cache::getSet_LRU(int cycle, uint64_t indx) {
    int set = 0;
    int oldestCycle = cycle;
    for (int i = 0; i < _sa; i++) {
        if (_cacheLines[i][indx].getCycle() < oldestCycle) {
		oldestCycle == _cacheLines[i][indx].getCycle();
		set = i;
	}
    }
    return set;
...
//  } else {
	//Direct Map cache
        //_cacheLines[0][indx].writeLine(tag, data); //TODO fix for sa if needed it
//  }
}
...
bool cache::isHit(uint64_t addr)
{
    uint64_t indx = getIndex(addr);
    uint64_t tag = getTag(addr);
    if (tag == 0) {printf("\nERROR: the tag has not been set\n");}
