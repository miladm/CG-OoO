...
void print_wire_energy(flp_t *flp, double frequency)
{
	int i, j;
	double energy_g, energy_i;
	fprintf(stdout, "printing wire energy between blocks for global and intermediate metal layers:\n");
	...
	for (i=0; i < flp->n_units-1; i++)
		for (j=i+1; j < flp->n_units; j++) 
			if (flp->wire_density[i][j]) {
				energy_g = wire_length2energy(get_manhattan_dist(flp, i, j), WIRE_GLOBAL);
...
int main(int argc, char **argv)
{
	...
	/* print the wire delays between blocks	*/
	print_wire_energy(flp, thermal_config.base_proc_freq);
