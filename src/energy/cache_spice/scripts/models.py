...
class CellModel:
    def __init__(self, **kwargs):
        ...
        self.w = kwargs["w"]
        self.h = kwargs["h"]
        self.rp = kwargs["rd_port"]
        self.wp = kwargs["wr_port"]

        assert (self.rp > 0), ("must have at least one read port")
        self.w = (2 * self.wp + self.rp) * self.w
        self.h = (self.wp + self.rp) * self.h
