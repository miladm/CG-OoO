...
class Array:
    ...
    def get_energy(self):
        ...
        leakage_energy = float(self.sram_stats["e_leak"])
        ...
        # hack for CAM energy
        wl_e *= 1e-9
        e_sl = (wl_e) * (self.decoder.Nh / 2)
        cam_energy = e_sl + total_energy
        print "SL Energy: ", e_sl
        print "CAM Energy: ", cam_energy
    ...
    def get_size(self):
        ...
        # multiple tables energy (assuming it only changes width)
        total_w *= self.Na
...
if __name__ == '__main__':
    ...
    area = 1
    for x in array.get_size():
        area *= (x * 1e3)
    print 'Area = ', ['%fe-6 m^2 ' %area]
