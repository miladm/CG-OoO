...
void livenessAnalysis (List<basicblock*> *bbList, REG_ALLOC_MODE reg_alloc_mode) {
    ...
    while (change == true) {
        ...
        for (int i = 0; i < bbList->NumElements (); i++) {	
            basicblock* bb = bbList->Nth (i);
            if (bb->update_InOutSet () == true) change = true;
        }
    }
...
void set_arch_reg_for_all_ins (basicblock* bb, map<long int,interfNode*> &allIntfNodeMap) {
	// printf ("3 map size: %d\n", allIntfNodeMap.size ());
	...
	for (int i = 0; i <  insList->NumElements (); i++) {
		...
		for (int j = 0; j < ins->getNumReg (); j++) {
			...
			if (allIntfNodeMap.find (ins->getNthReg (j)) == allIntfNodeMap.end ()) break; //TODO replace this with next line
